-- Fix delivery slots to ensure all upcoming delivery days show properly
-- The issue might be case sensitivity or the cutoff calculation
...
BEGIN
  ...
  dates_with_counts AS (
    ...
    WHERE 
      -- Only show if cutoff hasn't passed (use > instead of >= to be more lenient)
      ud.next_delivery_date > (local_date + COALESCE(ud.cutoff_days_before, 0) - 1)
    GROUP BY 
      ud.schedule_id, 
      ud.day_of_week, 
