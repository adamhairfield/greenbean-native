-- Fix promo code validation to bypass RLS when checking usage
-- The function needs SECURITY DEFINER to count all usage records for a user
...
BEGIN
    ...
    -- Check per-user usage limit (with SECURITY DEFINER, this will bypass RLS)
    IF v_promo.usage_limit_per_user IS NOT NULL THEN
        SELECT COUNT(*) INTO v_user_usage_count
        FROM promo_code_usage
        WHERE promo_code_usage.promo_code_id = v_promo.id
        AND promo_code_usage.user_id = p_user_id;
